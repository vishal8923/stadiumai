"""Seed database with initial mock datasets for development and testing."""
...
def _seed_transport(db: Session) -> None:
    transport_options = [
...
def _seed_accessibility(db: Session) -> None:
    services = [
...
def _get_crowd_level(density: float) -> str:
    if density >= 0.9:
        return "critical"
    if density >= 0.7:
        return "high"
...
def _seed_crowd_data(db: Session) -> None:
    now = datetime.datetime.now(datetime.timezone.utc)
    entries = []

    for zone in STADIUM_NODES:
        ...
        for diff_min in [30, 15, 0]:
            ...
            level = _get_crowd_level(density)
...
def _seed_knowledge_items(db: Session) -> None:
    json_files = {
        "faq": "faq.json",
...
def seed_database(db: Session) -> None:
    ...
    _seed_matches(db)
    _seed_transport(db)
    _seed_accessibility(db)
    _seed_staff(db)
    _seed_crowd_data(db)
    _seed_knowledge_items(db)
